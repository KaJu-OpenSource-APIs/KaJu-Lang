...
Os códigos são agrupados em faixas: `K0xx` (análise e semântica), `K1xx` (léxico) e `K2xx` (execução). A fronteira não é rígida — alguns erros que só aparecem ao rodar, como divisão por zero (`K020`), ficam na faixa `K0xx`/`K2xx` conforme a natureza. Todos têm explicação em `kaju explique <codigo>`.
